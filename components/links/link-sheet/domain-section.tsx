...
export default function DomainSection({
  ...
  const handleDomainChange = (value: string) => {
    ...
    if (value === "add_domain" || value === "add_dataroom_domain") {
      ...
      setModalOpen(true);
      setData({ ...data, domain: "getpixcel.com" });
      return;
    }
  ...
  useEffect(() => {
    if (domains && !editLink) {
      ...
      setData({
        ...data,
        domain: defaultDomain?.slug ?? "getpixcel.com",
      });
    }
  }, [domains, editLink]);

  const defaultDomain = editLink
    ? (data.domain ?? "getpixcel.com")
    : (domains?.find((domain) => domain.isDefault)?.slug ?? "getpixcel.com");
  ...
  return (
    <>
        ...
        >
          <SelectTrigger
            className={cn(
              ...
              data.domain && data.domain !== "getpixcel.com"
                ? ""
          ...
          <SelectContent className="flex w-full rounded-md border border-input bg-white text-foreground placeholder-muted-foreground focus:border-muted-foreground focus:outline-none focus:ring-inset focus:ring-muted-foreground dark:border-gray-500 dark:bg-gray-800 focus:dark:bg-transparent sm:text-sm">
            <SelectItem value="getpixcel.com" className="hover:bg-muted">
              getpixcel.com
            </SelectItem>
        ...
        {data.domain && data.domain !== "getpixcel.com" ? (
          <Input
            ...
            className={cn(
              ...
              data.domain && data.domain !== "getpixcel.com" ? "flex" : "",
            )}
            placeholder="deck"
            onChange={(e) => {
      ...
      {data.domain && data.domain !== "getpixcel.com" && !isDomainVerified ? (
